$.fn.GotechProgressIsInViewport = function(content) {
	"use strict";
...
(function($){
  "use strict";
	...
	var JamesGotech = {
		...
		init: function(){
			JamesGotech.BgImg();
			JamesGotech.imgToSVG();
			JamesGotech.progress();
			JamesGotech.resume();
			JamesGotech.loadBlogPosts();
			JamesGotech.modal();
			JamesGotech.progress2();
			JamesGotech.contactForm();
			JamesGotech.toTopJumper();
			JamesGotech.rating();
			JamesGotech.isotope();
			JamesGotech.portfolioFilter();
			JamesGotech.magnific();
			JamesGotech.anchor();
			JamesGotech.darklight();
		},
		
		darklight: function(){
			$('.gotech_fn_switcher_wrap input').on('change',function(){
				var checkBox = $(this);
		...
		progress2: function(){
			$('.fn_cs_progress_bar').each(function() {
				var pWrap 	= $(this);
				pWrap.waypoint({handler: function(){JamesGotech.progress2F(pWrap);},offset:'90%'});
			});
		},
		...
		modal: function(){
			var self		= this;
			var modalBox	= $('.gotech_fn_modalbox');
			var item		= $('.modal_item');
		...
		modal_prevnext: function(prevNext,modalBox){
			...
			prevNext.find('a').off().on('click',function(){
				...
				$(".gotech_fn_modalbox .modal_content").stop().animate({scrollTop:0}, 500, 'swing');
		...
		resume: function(){
			$('.gotech_fn__cv .cv__content').scrollTop(0);
			$('body').addClass('resume-opened');
		},
		
		progress: function(){
			var content = $('.gotech_fn__cv .cv__content');
			if($(window).width() <= 768){
				content = $('.gotech_fn__cv');
			}
			content.on('resize scroll', function() {
				if ($('.gotech_fn__cv .fn_cs_progress_bar').GotechProgressIsInViewport(content) < 0) {
					JamesGotech.progressF($('.gotech_fn__cv .fn_cs_progress_bar'));
				}
			});
		},
	...
	// READY Functions
	$(document).ready(function(){JamesGotech.init();});
